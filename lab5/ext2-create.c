...
void write_block_bitmap(int fd)
{
	...
	for(u32 b = 1; b <= LAST_BLOCK; b++){
		set_bit(bitmap, b);
	}

	for(u32 b = NUM_BLOCKS + 1; b < BLOCK_SIZE*8; b++){
		set_bit(bitmap, b);
	}
...
void write_inode_table(int fd) {
	...
	hello_world_inode.i_mode = EXT2_S_IFREG | EXT2_S_IRUSR | EXT2_S_IRGRP | EXT2_S_IROTH;
	...
	memcpy(&hello_inode.i_block, target_path, strlen(target_path));
...
void write_hello_world_file_block(int fd)
{
	...
	ssize_t size = (ssize_t)strlen(text);
